...
GRID = (200, 160)  # (nx, ny) for x∈[0,50] m, y∈[-20,20] m @ 0.25 m/cell
...
def test_fusion_guard_rejects_misaligned_grids():
    """The §02 guard fires when the two BEV maps don't share the grid."""
    detector = BEVDetector(BEVFusionConfig())  # defaults: 64 / 128 ch, 200×160
...
def _panel(ax, feat, title, cmap, x_range, y_range, centres=None):
    ...
    # Lock the view to the BEV extent so out-of-grid scatter can't rescale it.
    ax.set_xlim(y_range)
    ax.set_ylim(x_range)
...
def save_fusion_figure(save_path: str = "docs/img/bev_fusion_test_output.png") -> str:
    ...
    fig.suptitle(
        f"BEV fusion (untrained) | {sample.dataset} log={sample.log_name} iter={sample.iteration}",
        fontsize=13, fontweight="bold",
    )
    _panel(axes[0], _aggregate(bev_camera), "Camera BEV (64 ch, |mean|)", "plasma", x_range, y_range)
    _panel(axes[1], _aggregate(bev_lidar), "LiDAR BEV (128 ch, |mean|)", "viridis", x_range, y_range)
    _panel(axes[2], _aggregate(fused[0]), "Fused BEV (128 ch, |mean|)", "magma", x_range, y_range, centres)
    _panel(axes[3], heatmap, "Head heatmap (max class, σ)", "inferno", x_range, y_range, centres)
